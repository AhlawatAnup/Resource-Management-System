...
document.addEventListener('DOMContentLoaded', () => {
  ...
  const uiRefs = ui.initUI({
    ...
    onAddClick: () => {
      const modal = ui.ensureAddModal(handleAddSubmit);
  ...
  async function handleImport(e) {
    ...
    try {
      ...
      Toastify({ text: 'Import failed: ' + err.message, duration: 3000, gravity: "top", position: "center", backgroundColor: "#ff6b6b" }).showToast();
    } finally {
  ...
  // ---------------- DELETE ----------------
  async function handleDelete(machine, tr) {
    if (utils.getAssignedStatus(machine)) {
      Toastify({ text: "Cannot delete machine that is assigned.", duration: 3000, gravity: "top", position: "center", backgroundColor: "#ff6b6b" }).showToast();
      return;
    }
  ...
  // ---------------- REVOKE ----------------
  async function handleRevoke(machine, assignedTd, revokeBtn) {
    ...
    try {
      ...
      Toastify({ text: err.message, duration: 3000, gravity: "top", position: "center", backgroundColor: "#ff6b6b" }).showToast();
    }
  }

  // ---------------- EDIT ----------------
  function handleEdit(machine, tr) {
    if (utils.getAssignedStatus(machine)) {
      Toastify({ text: "Cannot edit assigned machine", duration: 3000, gravity: "top", position: "center", backgroundColor: "#ff6b6b" }).showToast();
      return;
    }
  ...
  async function handleEditSubmit({ id, MIGID, gpuRaw }) {
  ...
  // ---------------- ADD ----------------
  async function handleAddSubmit({ MIGID, gpuRaw }) {
    const { valid, value: gpu } = utils.validateGpu(gpuRaw);
