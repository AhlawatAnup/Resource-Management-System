...
function setStudentFieldState(isStudent) {
  ...
  studentInputs.forEach((el) => {
    if (isStudent) {
      el.disabled = false;
      if (
        el.id === 'assignedTeacher' ||
        el.id === 'rollNumber' ||
        el.id === 'instituteName' ||
        el.id === 'instituteAddress'
      ) {
        el.required = true;
      }
    } else {
...
document.getElementById('registrationForm').addEventListener('submit', async function (e) {
  ...
  if (role === 'student') {
    ...
    if (!selectedTeacher) {
      ...
      submitBtn.innerHTML = originalText;
      Toastify({
        text: 'Please select a teacher to proceed with registration.',
        duration: 3000,
        gravity: 'top',
        position: 'center',
        backgroundColor: '#ff6b6b',
    ...
    if (!/^[A-Za-z0-9]+$/.test(rollNo)) {
      Toastify({
        text: 'Roll number should only contain letters and numbers.',
        duration: 3000,
        gravity: 'top',
        position: 'center',
        backgroundColor: '#ff6b6b',
