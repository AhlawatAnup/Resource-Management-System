...
/**
 * Generate undertaking PDF as a buffer
 * @param {Object} studentData - Student information
 * @param {string} studentData.name - Student name
 * @param {string} studentData.rollNo - Roll number
 * @param {string} studentData.branch - Branch/Department
 * @param {string} studentData.instituteName - Institute name
 * @param {string} studentData.instituteAddress - Institute address
 * @param {string} purpose - Purpose of resource request
    ...
    return new Promise((resolve, reject) => {
        try {
            ...
            doc.fontSize(14).font('Helvetica-Bold').text(
                'Undertaking for Use of AI Data Centre Facilities at Panjab University, Chandigarh',
                { align: 'center' }
            );
            doc.moveDown(1.5);
            ...
            doc.moveDown(0.5);
            doc.text('• Ministry of Education (MoE), Government of India, at the Design Innovation Centre (DIC) (File No.: 17-11/2015-PN-1), November 2015.', { indent: 20 });
            doc.text('• Department of Science and Technology (DST) through the Fund for Improvement of S&T Infrastructure (FIST) grant (SR/FST/ET-I/2021/878), 2021.', { indent: 20 });
            doc.text('• AI Data Centre established through CSR grant by Vardhman Textile Pvt Ltd and Intel Corp by Design Innovation Centre, Panjab University, Chandigarh. (File No: 05/02/2025)', { indent: 20 });
            doc.moveDown(0.5);
            doc.text(
            ...
            doc.font('Helvetica-Bold').text('User Details', { underline: true });
            doc.moveDown(0.5);
            ...
            doc.font('Helvetica-Oblique').text(`Signed by ${studentData.name || 'User'}`, valueStartX, doc.y - 14);
